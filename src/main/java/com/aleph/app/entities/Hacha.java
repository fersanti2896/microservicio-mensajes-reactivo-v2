...
public class Hacha {
	...
	public Hacha(String rutaOrigen, String rutaDestino, int UMA, String nombreArchivo, String token) {
		this.UMA = UMA;
	...
	public void crearCortes() throws IOException {
		...
		while ((bytesLeidos = seekableBCLectura.read(byteBuffer)) > 0) {
			...
			if (contadorChunks == 0) {
				...
				String Recibirtoken = token;
				HttpHeaders headers = new HttpHeaders();
				headers.setContentType(MediaType.MULTIPART_FORM_DATA);
				headers.set("Authorization", "Bearer" + Recibirtoken);
				MultiValueMap<String, Object> body = new LinkedMultiValueMap<>();
				body.add("file", new FileSystemResource(pathCorte));
